...
echo ""
echo "[uDOS-ubuntu] What the operator gets (readable)"
python3 "$HR_PY" "$MANIFEST"
echo ""

if [ "${UDOS_DEMO_INCLUDE_RAW_JSON:-0}" = "1" ]; then
  echo "[uDOS-ubuntu] Raw manifest JSON"
  cat "$MANIFEST"
else
  echo "[uDOS-ubuntu] Machine-readable manifest: examples/browser-workstation-scaffold.json"
