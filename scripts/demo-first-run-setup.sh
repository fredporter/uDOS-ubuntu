...
echo ""
echo "[uDOS-ubuntu] Browser workstation — operator surface (readable)"
python3 "$HR_PY" "$REPO_ROOT/examples/browser-workstation-scaffold.json"
echo ""
echo "[uDOS-ubuntu] ThinUI — operator surface (readable)"
python3 "$HR_PY" --kind thinui "$REPO_ROOT/examples/thinui-c64-launch.json"
echo ""

if [ "${UDOS_DEMO_INCLUDE_RAW_JSON:-0}" = "1" ]; then
  echo "[uDOS-ubuntu] Raw contract JSON (browser workstation)"
  cat "$REPO_ROOT/examples/browser-workstation-scaffold.json"
  echo ""
  echo "[uDOS-ubuntu] Raw contract JSON (ThinUI)"
  cat "$REPO_ROOT/examples/thinui-c64-launch.json"
  echo ""
else
  echo "[uDOS-ubuntu] Contracts on disk: examples/browser-workstation-scaffold.json, examples/thinui-c64-launch.json"
...
if command -v node >/dev/null 2>&1 && [ -f "$THINUI_DEMO" ]; then
  ...
  echo "[uDOS-ubuntu] ThinUI sibling demo skipped (optional Node + uDOS-thinui repo)."
  if ! command -v node >/dev/null 2>&1; then
    echo "  Need: Node.js on PATH (e.g. apt install nodejs, or use linux-family-bootstrap.sh)."
  elif [ ! -f "$THINUI_DEMO" ]; then
    echo "  Need: optional repo uDOS-thinui next to uDOS-ubuntu (expected: $THINUI_DEMO)."
  fi
fi
